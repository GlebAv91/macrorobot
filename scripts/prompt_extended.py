...
def load_index_data_prompt():
    with open("output/index_macro_data_prompt.json", "r") as file:
        data = json.load(file)
...
response = client.models.generate_content(
    ...
    contents=(
        "You are experienced investment analyst and macroeconic data analyst. "
        + contents
        + "Based on provided index and macro data. Predict next 6 months close prices starting from the current month for all indices. "
        + "Ensure that the predictions use only the last business day of each month as the date, "
...
output_file = "output/model_extended_response.json"
